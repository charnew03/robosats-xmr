...
- Phase 3 (bonds + basic hardening) — IN PROGRESS (bonds + subaddress improvements; first slice complete: bonds, sweeper, collaborative cancel; see `docs/TESTING.md`).
...
- **Subaddress + polling improvements** — trade stores subaddress indexes and bond confirmation counters; wallet adapter reconciliation uses `get_transfers`-style activity polling.
- **Bond accounting checks** — settlement actions verify bond amounts, bond addresses, and resolved subaddress indexes before allowing `mark-fiat-paid` / `release-escrow`.
...
- **Collaborative cancel / release bond returns** — optional maker/taker bond-return wallet sends on pre-funding cancel and successful release, with audit txid notes.
- **Dispute bond policy placeholder** — dispute opening logs a coordinator-retains-bonds placeholder event (`bond_slash_placeholder`) for later policy implementation.
...
- Dev fake-wallet mode (default in compose): `docker compose up --build api`
- API available at: `http://127.0.0.1:8000`
- Real-mode stack (stagenet local services): `docker compose up --build`
  - `monerod` RPC: `http://127.0.0.1:18081`
  - `wallet-rpc` JSON-RPC: `http://127.0.0.1:18083`
  - To use real wallet mode in API, set `ROBOSATS_XMR_USE_FAKE_WALLET=false` for the API service/environment.
...
- ✅ Phase 1 — COMPLETE | ✅ Phase 2 — COMPLETE | Phase 3 (bonds + basic hardening) — IN PROGRESS (bonds + subaddress improvements)
